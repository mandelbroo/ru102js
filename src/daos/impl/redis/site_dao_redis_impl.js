...
const findAll = async () => {
  ...
  const siteNames = await client.smembersAsync(keyGenerator.getSiteIDsKey());

  const pipe = client.multi();
  siteNames.forEach(name => pipe.hgetall(name));
  const sites = await pipe.execAsync();

  return sites.map(remap);
  // END CHALLENGE #1
};
/* eslint-enable */
